...
#define SE_RTC_SET_WAKE_TIME	_IOW('p',  0xf1, u8)

#define RX8900_MAX_TEST_WAKE_TIME	(100)
static u8 g_test_wake_time = 0;
//----------------------------------------------------------------------SE_RTC_REG_READ
// rx8900_read_reg()
...
static int rx8900_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case SE_RTC_SET_WAKE_TIME:
			if (copy_from_user(&g_test_wake_time, argp, sizeof(g_test_wake_time)))
				return -EFAULT;
			if (g_test_wake_time > RX8900_MAX_TEST_WAKE_TIME) {
				g_test_wake_time = 0;
				dev_err(&client->dev, "invalid wake time %d\n", g_test_wake_time);
				return -EFAULT;
			}
			dev_info(&client->dev, "set test wake time=%d\n", g_test_wake_time);
			ret = 0;
			break;
...
#define SHUTDOWN_TIME_MARGIN 3
	...
	if (g_test_wake_time == 0 && seconds_alarm_values <= 0)
		return;
	...
	if (g_test_wake_time) {
		...
		alarm_secs = cur_secs + g_test_wake_time;
		seconds_alarm_values = g_test_wake_time;
	}
	...
	if (seconds_alarm_values < RX8900_FIE_MAX_TIME && cur_secs < alarm_secs) {
		...
		dev_warn(&client->dev, "Alarm time %lld is less than current time %lld, "
