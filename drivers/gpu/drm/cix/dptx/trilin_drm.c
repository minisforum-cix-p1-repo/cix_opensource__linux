					    ...
					    struct drm_atomic_state *state)
{
	...
	*/
	...
	if (!drm_connector_atomic_hdr_metadata_equal(old_con_state,
		...
		DP_DEBUG("metadata changed between 0 and validate value. Do not change mode.");
	}
			       ...
			       struct drm_display_mode *mode)
{
	...
	/*Fixme*/
	if (mode->vdisplay == 2880 && mode->hdisplay == 2560) {
		DP_DEBUG("filtered mode(%s) for height 2880\n",
			mode->name);
		return MODE_CLOCK_HIGH;
	}

	//DP_DEBUG("mode name %s [%d x %d], mode->clock %d, rate %d\n",
	//		mode->name, mode->hdisplay, mode->vdisplay, mode->clock, rate);
	return MODE_OK;
}
...
static u8 trilin_dp_cal_bpp(u8 bpc, enum trilin_dpsub_format format)
{
	u8 bpp = bpc * 3;
					   ...
					   struct drm_display_mode *adjusted_mode)
{
	...
	DP_DEBUG("adjusted_mode->vsync_start  %d  %d %d %d"
		, adjusted_mode->vsync_start, adjusted_mode->vdisplay, extension, adjusted_mode->vtotal);
	adjusted_mode->vsync_start += extension;
	adjusted_mode->vsync_end += extension;
	adjusted_mode->vtotal += extension;
	vrefresh = adjusted_mode->clock * 1000 / (adjusted_mode->vtotal * adjusted_mode->htotal);
	DP_DEBUG("vmin: %d vmax:%d VRR enable adjusted_mode->vtotal adjust form %d to %d vrefresh=%d"
		, vmin, vmax, adjusted_mode->vtotal - extension, adjusted_mode->vtotal, vrefresh);
#endif
		...
		struct drm_display_mode *adjusted_mode)
{
	...
	if (diff < TRILIN_DPTX_MIN_H_BACKPORCH) {
		...
		if (adjust_backporch) {
			...
			adjusted_mode->clock = (int64_t)adjusted_mode->clock * adjusted_mode->htotal / (adjusted_mode->htotal - diff);
		}
		DP_WARN("Note: hbackporch should adjust: %d to %d ? %s\n"
			, diff, TRILIN_DPTX_MIN_H_BACKPORCH - diff, adjust_backporch ? "YES" : "NO");
	}
				  ...
				  u8 suggest_bpc)
{
	...
	case DRM_COLOR_FORMAT_YCBCR420:
		if (connector_state->connector->ycbcr_420_allowed &&
	...
	min_bpc = (format == TRILIN_DPSUB_FORMAT_RGB) ? 6 : 8;
	bpc = max(suggest_bpc, min_bpc);

	for (; bpc >= min_bpc; bpc = bpc - 2) {
		bpc = trilin_dp_cal_bpc(dp, connector_state, bpc);
		bpp = trilin_dp_cal_bpp(bpc, format);
		...
		if (adjusted_mode->clock <= rate) {
			...
			dev_err(dp->dev, "mode %s pixel rate %d is higher than max rate %d\n",
				adjusted_mode->name, adjusted_mode->clock, rate);
			return -EINVAL;
		}
	}

	DP_DEBUG("Note: disp->bpc(%d) bpc(%d) bpp(%d) colorspace=%d clock=%d color_format=%0x info->color_formats=%0x\n",
				info->bpc, bpc, bpp, colorspace, adjusted_mode->clock, color_format, info->color_formats);
				  ...
				  struct drm_connector_state *connector_state)
{
	...
	return trilin_dp_encoder_compute_config(encoder, crtc_state, connector_state, info->bpc);
}
