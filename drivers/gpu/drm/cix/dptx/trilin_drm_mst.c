					  ...
					  struct drm_connector_state *conn_state)
{
	...
	while (true) {
		...
		mst_encoder->pbn = drm_dp_calc_pbn_mode(adjusted_mode->clock, conn->config.bpp, false);
...
int trilin_drm_mst_encoder_init(struct trilin_dp *dp, int conn_base_id)
{
	...
	for (i = 0; i < MAX_DP_MST_DRM_ENCODERS; i++) {
		...
		drm_encoder_init(dp->drm, encoder, &trilin_dp_mst_enc_funcs,
