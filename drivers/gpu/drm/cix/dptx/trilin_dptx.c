...
#endif
	DP_DEBUG("training pattern is %0x", pat);
	if (ret < 0)
		return ret;
...
int trilin_dp_train_loop(struct trilin_dp *dp)
{
	...
	for (try = 0; try < DP_MAX_TRAINING_LOOP; try++) {
		...
		trilin_dp_mode_configure(dp, mode->pclock, 0, bpp);
	}
				      ...
				      struct drm_display_mode *mode, int bpp)
{
	...
	DP_DEBUG(
		"trilin_dp_mode_set_timing adjust_mode flags: 0x%0x lane_cnt=%d mode.lans=%d (htotal=%d vtotal=%d) data_control=0x%0x",
					       ...
					       bool enable)
{
	...
	DP_DEBUG("%s stream%d %s", enable ? "enable" : "disable",
		 dp_panel->stream_id,
...
static int trilin_dp_core_on(struct trilin_dp *dp, bool shallow)
{
	...
	trilin_dp_write(
		dp, TRILIN_DPTX_VIDEO_STREAM_ENABLE + TRILIN_DPTX_SOURCE_OFFSET,
		0);
...
static int trilin_dp_core_off(struct trilin_dp *dp)
{
	...
	memset(&(dp->mst_ch_info.slot_info), 0,
	       sizeof(struct trilin_dp_mst_ch_slot_info) * DP_STREAM_MAX);
...
static int trinlin_dp_panel_read_sink_caps(struct trilin_dp *dp)
{
	...
	DP_INFO("mst cap %d rlen=%d max_lanes=%d max_rate=%d sink_count=%d tps3_supported=%d, tps4_supported=%d vrr=%d arr=%d\n",
...
static bool trilin_dp_link_process_link_status_update(struct trilin_dp *dp)
{
	...
	if (!clock_recovery_ok || !channel_eq_ok) {
		DP_ERR("clock_recovery_ok[%d], chanel_eq_ok[%d]",
		       clock_recovery_ok, channel_eq_ok);
		return true;
	}
...
{
	DP_DEBUG(
		"trilin_dp_is_ready: hpd=%d state=%d sink count:%d is_sink_count_zero=%d\n",
...
void trilin_dp_dump_regs(struct seq_file *m, struct trilin_dp *dp)
{
	...
	if (dp->mst_mgr != NULL) {
		seq_printf(m,
			   "----Main stream control, virtual source 1----\n");
		for (i = TRILIN_DPTX_SRC1_VIDEO_STREAM_ENABLE;
		     i <= TRILIN_DPTX_SRC1_FRAMING_STATUS; i += 4)
			DUMP_DP_REG(i);
		seq_printf(m, "----Secondary channel, source 1----\n");
		for (i = TRILIN_DPTX_SEC1_AUDIO_ENABLE;
		     i <= TRILIN_DPTX_SEC1_ADAPTIVE_SYNC_ENABLE; i += 4)
			DUMP_DP_REG(i);
	}
}
