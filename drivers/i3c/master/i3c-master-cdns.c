...
#define CDNS_I3C_PM_TIMEOUT_MS 10
...
struct cdns_i3c_store_reg {
	u32 reg;
	u32 value;
};

struct cdns_i3c_store_reg  need_save_reg[SAVE_MAX_REG_SIZE];
...
static int __maybe_unused cdns_i3c_suspend(struct device *dev)
{
	...
	need_save_reg[CTRL_INDEX].value = readl(master->regs + CTRL);
	need_save_reg[CTRL_INDEX].reg   = CTRL;

	need_save_reg[PRESCL_CTRL0_INDEX].value = readl(master->regs + PRESCL_CTRL0);
	need_save_reg[PRESCL_CTRL0_INDEX].reg   = PRESCL_CTRL0;

	need_save_reg[PRESCL_CTRL1_INDEX].value = readl(master->regs + PRESCL_CTRL1);
	need_save_reg[PRESCL_CTRL1_INDEX].reg   = PRESCL_CTRL1;

	need_save_reg[MST_IDR_INDEX].value = 0xFFFFFFFF;
	need_save_reg[MST_IDR_INDEX].reg   = MST_IDR;

	need_save_reg[SLV_IDR_INDEX].value = 0xFFFFFFFF;
	need_save_reg[SLV_IDR_INDEX].reg   = SLV_IDR;

	need_save_reg[CMD_IBI_THR_CTRL_INDEX].value = readl(master->regs + CMD_IBI_THR_CTRL);
	need_save_reg[CMD_IBI_THR_CTRL_INDEX].reg   = CMD_IBI_THR_CTRL;

	need_save_reg[MST_IER_INDEX].value = MST_INT_IBIR_THR;
	need_save_reg[MST_IER_INDEX].reg   = MST_IER;

	need_save_reg[DEVS_CTRL_INDEX].value = readl(master->regs + DEVS_CTRL);
	need_save_reg[DEVS_CTRL_INDEX].reg   = DEVS_CTRL;

	need_save_reg[TX_RX_THR_CTRL_INDEX].value = readl(master->regs + TX_RX_THR_CTRL);
	need_save_reg[TX_RX_THR_CTRL_INDEX].reg   = TX_RX_THR_CTRL;

	while (rrx < (DEV_ID6_RR2_INDEX - DEV_ID0_RR0_INDEX)) {

		need_save_reg[DEV_ID0_RR0_INDEX + rrx].value = readl(master->regs + DEV_ID_RR0(rrx/3));
		need_save_reg[DEV_ID0_RR0_INDEX + rrx].reg   = DEV_ID_RR0(rrx/3);
		need_save_reg[DEV_ID0_RR0_INDEX + rrx + 1].value = readl(master->regs + DEV_ID_RR1(rrx/3));
		need_save_reg[DEV_ID0_RR0_INDEX + rrx + 1].reg   = DEV_ID_RR1(rrx/3);
		need_save_reg[DEV_ID0_RR0_INDEX + rrx + 2].value = readl(master->regs + DEV_ID_RR2(rrx/3));
		need_save_reg[DEV_ID0_RR0_INDEX + rrx + 2].reg   = DEV_ID_RR2(rrx/3);
		rrx += 3;
	}
...
static int __maybe_unused cdns_i3c_resume(struct device *dev)
{
	...
	while (reg_num < SAVE_MAX_REG_SIZE) {
		writel(need_save_reg[reg_num].value, master->regs + need_save_reg[reg_num].reg);
		reg_num++;
	}
