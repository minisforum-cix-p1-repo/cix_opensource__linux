...
#define CIX_IPBLOQ_POWER_SAVE_DEFAULT_TIME_MS	0
...
/* alc269 mg pl init verb table */
static unsigned int alc269_mg_p1wsb_init_verbs[] = {
        /* Realtek High Definition Audio Configuration - Version : 5.0.3.3
...
static int cix_ipbloq_hda_config_init_verbs(struct hdac_bus *bus, unsigned int vendor_id)
{
	...
	case 0x10ec0256:
		...
		break;
	case 0x10ec0269:
		init_verbs = alc269_mg_p1wsb_init_verbs;
		size = ARRAY_SIZE(alc269_mg_p1wsb_init_verbs);
		break;
	default:
...
static int cix_ipbloq_hda_init(struct azx *chip, struct platform_device *pdev)
{
	...
	/* shortname for card */
	sname = of_get_property(np, "cix,model", NULL);
	if (!sname)
		sname = drv_name;
